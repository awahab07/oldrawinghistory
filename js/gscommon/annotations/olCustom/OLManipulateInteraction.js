define([
    "dojo/_base/lang",
    "dojo/_base/array",
    "gscommon/annotations/olCustom/OLManipulationLayer"
], function (lang, array, OLManipulationLayer) {
    ...
    ol.interaction.Manipulate = function(opt_options) {
        ...
        this.dragStarted_ = false; // Flag to determine whether a drag is done (before an UP Event).
        ...
        this.handleMapBrowserEventForSelect_ = function(mapBrowserEvent) {
            ...
            if(!goog.isNull(this.draggingFeature_)) {
                ...
                // Route the DB Click to Manipulation Layer for feature
                if(mapBrowserEvent.type === ol.MapBrowserEvent.EventType.DBLCLICK) { // Don't account drag and up events if DB Click is handled by selected feature
                    // featureDBClicked will return boolean indicating whether event is handled
                    if(this.manipulationLayer_.featureDBLClicked(this.map_, this.draggingFeature_, this.dragFromPx_, mapBrowserEvent.pixel, mapBrowserEvent)) {
                        return false;
                    }
                }

                if(!goog.isNull(this.dragFromPx_)) {
                    ...
                    if (mapBrowserEvent.type === ol.MapBrowserEvent.EventType.POINTERDRAG) {
                        ...
                        // If its a drag start
                        if (!this.dragStarted_ && this.apiRef_) {
                            // Triggering api that shape is going to be modified
                            this.apiRef_.beforeFeatureModified(this.manipulationLayer_.getManipulatingFeature());
                        }
                    ...
                    if (mapBrowserEvent.type == ol.MapBrowserEvent.EventType.POINTERUP) {
                        // If drag is done, trigger the api that shape has been modified
                        if (this.dragStarted_ && this.apiRef_) {
                            this.apiRef_.featureModified(this.manipulationLayer_.getManipulatingFeature());
                        }
        ...
        this.shouldRouteToSelect_ = function(mapBrowserEvent) {
            ...
            if(	mapBrowserEvent.type === ol.MapBrowserEvent.EventType.POINTERDRAG ||
                ...
                mapBrowserEvent.type === ol.MapBrowserEvent.EventType.DBLCLICK) {
                return true;
            }
    ...
    ol.interaction.Manipulate.prototype.clear = function(){
        if(this.manipulationLayer_) {
            this.manipulationLayer_.clear();
        }
    };

    ol.interaction.Manipulate.prototype.setActive = function(active) {
        goog.base(this, 'setActive', active);

        this.clear();
    };

    ol.interaction.Manipulate.prototype.deleteSelectedFeatures = function(layer) {
        // API can provide 'layer' to delete feature on
        // It may be required to delete the selected features on provided 'layer' only

        // For now use the interaction managed layer to delete the selected feature
        if(this.layerToManipulateOn && this.draggingFeature_) {
            if(this.apiRef_) { this.apiRef_.beforeFeatureDeleted(this.draggingFeature_); }
            this.layerToManipulateOn.getSource().removeFeature(this.draggingFeature_);
            if(this.apiRef_) { this.apiRef_.featureDeleted(this.draggingFeature_); }
        } else if(layer  && this.draggingFeature_) {
            if(this.apiRef_) { this.apiRef_.beforeFeatureDeleted(this.draggingFeature_); }
            layer.getSource().removeFeature(this.draggingFeature_);
            if(this.apiRef_) { this.apiRef_.featureDeleted(this.draggingFeature_); }
        }
    ...
    return ol.interaction.Manipulate;
});