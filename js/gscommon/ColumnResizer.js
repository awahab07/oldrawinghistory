/**
 * Created by muhammad.shahzad on 12/27/2014.
 */
define(["dojo/_base/declare", "dojo/on", "dojo/query", "dojo/_base/lang", "dojo/dom", "dojo/dom-geometry", "dojo/has", "dgrid/util/misc", "put-selector/put", "dojo/_base/html", "xstyle/css!dgrid/css/extensions/ColumnResizer.css"],
...
// Functions for shared resizer node
        ...
        return declare(null, {
            ...
            _resizeMouseUp: function (e) {
                ...
                var noOfVisibleColumns = 0;
                var startIndex = 0, col, l;
                for (col = 0, l = colNodes.length; col < l; col++) {
                    if (colNodes[col].field == cell.field) {
                        startIndex = col + 1;
                    }
                }
                for (col = startIndex, l = colNodes.length; col < l; col++) {
                        if (!this.columns[colNodes[col].field].hidden) {
                            noOfVisibleColumns++;
                        }
                }
                var colSub = -1 * (delta / (noOfVisibleColumns ));
                for (col = startIndex, l = colNodes.length; col < l; col++) {
                    if (!this.columns[colNodes[col].field].hidden) {
                        if (colNodes[col].columnId != cell.columnId)
                        {
                            var newWidthToSet=colNodes[col].offsetWidth + colSub;
                            var currentWidth=colNodes[col].offsetWidth;
                            if(newWidthToSet<this.minWidth){
                                newWidth-=(currentWidth-this.minWidth);
                                newWidthToSet=this.minWidth;
                            }
                            resizeColumnWidth(this, colNodes[col].columnId, colNodes[col].offsetWidth + colSub, e.type);
                        }
                    }
                }
