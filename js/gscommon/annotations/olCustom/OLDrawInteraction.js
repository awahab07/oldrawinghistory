define([
    ...
    ol.interaction.DrawWithShapes = function(options) {
        ...
        this.apiRef_ = options.apiRef; // to ShapeDrawingMixin
        ...
        this.addActivityHook = function(hookType, hookFn, args) {
            if(!this.activityHooks_[hookType]) {
                this.activityHooks_[hookType] = [];
            }
            this.activityHooks_[hookType].push([hookFn, args]);
        };

        this.deactivateDrawing = function() {
            if(this.apiRef_ && this.apiRef_.drawingDeactivated) {
                this.apiRef_.drawingDeactivated(/*string*/this.shapeType_);
            }
        ...
        this.isSingleClickDrawing_ = function() {
            var isSingleClickDrawing = false;

            if(this.shapeDrawingClass_ && this.shapeDrawingClass_.prototype.isSingleClickDrawing) {
                isSingleClickDrawing = this.shapeDrawingClass_.prototype.isSingleClickDrawing();
            }
            return isSingleClickDrawing;
        };

        this.isSingleAndPressDragReleaseDrawing_ = function() {
            return this.shapeDrawingClass_ && this.shapeDrawingClass_.prototype.isSingleAndPressDragReleaseDrawing
                && this.shapeDrawingClass_.prototype.isSingleAndPressDragReleaseDrawing();
        };

        this.createNewSketchFeature_ = function() {
            var currentStyleObj = this.getCurrentStyleObject();

            if(this.shapeType_ && this.shapeDrawingClass_) {
                return this.shapeDrawingClass_.prototype.createNewSketchFeature_(currentStyleObj);
            } else {
                return new ol.shape.ShapeFeature();
            }
        };
        this.addColorProvider = function(provider){this.colorProvider_ = provider}; // For testing/demo only
        this.getCurrentStyleObject = function() {
            // Applying currently selected style

            var styleObj = false;

            if(this.colorProvider_) {
                var fillColor = this.colorProvider_.get('color'),
                    strokeColor = this.colorProvider_.get('stroke'),
                    strokeWidth = this.colorProvider_.get('lineWidth') && Number(this.colorProvider_.get('lineWidth').value) || 1,
                    strokeStyle = Number(this.colorProvider_.get('lineStyleValue')),
                    textFill = this.colorProvider_.get('textFill'),
                    opacity = this.colorProvider_.get('opacity');

                styleObj = {
                    fill: {
                        color: fillColor,
                        opacity: opacity // @TODO move the opacity to its own key
                    },
                    stroke: {
                        color: strokeColor,
                        width: strokeWidth,
        ...
        this.addCreatedFeatureToDrawingLayer_ = function(createdFeature, drawingCoordinates) {
            ...
            if(createdFeature.drawingCompleted) {
                createdFeature.drawingCompleted(createdFeature, drawingCoordinates, this.apiRef_);
            }

            var styleObj = this.getCurrentStyleObject();
            if(styleObj) {
                createdFeature.updateStyle(styleObj);
            }

            // Triggering the api that feature is created
            if(this.apiRef_) {
                this.apiRef_.featureCreated(createdFeature);
            }
        };

        this.startShapeDrawing_ = function(event) {
        ...
        this.updateShapeDrawing_ = function(event){
            this.finishCoordinate_ = event.coordinate;
        ...
        this.finishShapeDrawingIfStarted_ = function(event){
            var pass = true;
        ...
        this.handleMapBrowserEvent = function(event) {
            ...
            if(this.isSingleAndPressDragReleaseDrawing_()) {
                if(event.type === ol.MapBrowserEvent.EventType.POINTERDRAG || event.type === ol.MapBrowserEvent.EventType.POINTERUP) {
                    if(goog.isNull(this.sketchFeature_)) {
                        // Start Drawing
                        this.startShapeDrawing_(event);
                    }
                }

                if(event.type === ol.MapBrowserEvent.EventType.POINTERUP) {
                    pass = this.finishShapeDrawingIfStarted_(event);
                }

                if(event.type === ol.MapBrowserEvent.EventType.POINTERDRAG && !goog.isNull(this.sketchFeature_)) {
                    this.updateShapeDrawing_(event);
                    pass = false;
                }
            } else if (this.isSingleClickDrawing_()) {
                if(event.type === ol.MapBrowserEvent.EventType.POINTERUP) {
            ...
            } else if (this.isPressDragReleaseDrawing_()) {
    ...
    return ol.interaction.DrawWithShapes;
});