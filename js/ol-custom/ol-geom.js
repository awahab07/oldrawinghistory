...
// 11 ol.geom.Ellipse
ol.geom.Ellipse = function(center, opt_radiusX, opt_radiusY, opt_layout) {
  goog.base(this);
  var radiusX = goog.isDef(opt_radiusX) ? opt_radiusX : 0,
...
ol.geom.Ellipse.prototype.getExtent = function(opt_extent) {
  if (this.extentRevision != this.getRevision()) {
    var flatCoordinates = this.flatCoordinates;
    var radiusX = flatCoordinates[this.stride] - flatCoordinates[0],
    	radiusY = flatCoordinates[this.stride*2 + 1] - flatCoordinates[1];
    this.extent = ol.extent.createOrUpdate(
        flatCoordinates[0] - radiusX, flatCoordinates[1] - radiusY,
        flatCoordinates[0] + radiusX, flatCoordinates[1] + radiusY,
        this.extent);
...
ol.geom.Ellipse.prototype.setCenter = function(center) {
  ...
  var radiusX = this.flatCoordinates[stride] - this.flatCoordinates[0];
  var radiusY = this.flatCoordinates[stride*2 + 1] - this.flatCoordinates[1];
  var flatCoordinates = center.slice();
  flatCoordinates[stride] = flatCoordinates[0] + radiusX;
  flatCoordinates[stride*2] = flatCoordinates[0];
  flatCoordinates[stride*2 + 1] = flatCoordinates[1] + radiusY;
  var i;
  for (i = 1; i < stride; ++i) {
  ...
  while (i < ii) {
    ...
    switch (type) {
      ...
      case ol.render.canvas.Instruction.ELLIPSE:
        goog.asserts.assert(goog.isNumber(instruction[1]));
        d = /** @type {number} */ (instruction[1]);
        var centerX = pixelCoordinates[d];
        var centerY = pixelCoordinates[d + 1];
        var width = Math.abs(pixelCoordinates[d + 2] - centerX);
        var height = Math.abs(pixelCoordinates[d + 5] - centerY);
        context.save();
